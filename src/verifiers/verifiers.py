...
class Verifiers:
        def __init__(self, p1, p2):
            ...
            self.pattern1 = p1
            self.pattern2 = p2
            self.common_features = set(self.pattern1.keys()).intersection(set(self.pattern2.keys()))
        def get_abs_match_score(self): # A verifier
            ...
            for feature in self.common_features: # checking for every common feature for match
                ...
                ratio = max(pattern1_mean, pattern2_mean) / min(pattern1_mean, pattern2_mean)
        ...
        def get_similarity_score(self): # S verifier, each key same weight
            ...
            for feature in self.common_features:
                ...
                try:
                    ...
                    pattern1_stdev = self.pattern1[feature] / 4  # this will always be one value that is when exception would occur

                value_matches, total_values = 0, 0
                for time in self.pattern2[feature]:
                    if (pattern1_mean - pattern1_stdev) < time < (pattern1_mean + pattern1_stdev):
                        value_matches += 1
                    total_values += 1
        ...
        def get_weighted_similarity_score(self):  # S verifier, each feature different weights
            ...
            for feature in self.common_features:
                ...
                try:
                    ...
                    template_stdev = self.self.pattern1[feature] / 4  # this will always be one value that is when exception would occur
                for time in self.pattern2[feature]:
                    if (enroll_mean - template_stdev) < time < (enroll_mean + template_stdev):
                        matches += 1
                    total += 1
...
# local testing
pattern1 = {"W":[210, 220, 200, 230], "E":[110, 115, 107], "L":[150, 130, 190, 120], "C":[25, 30, 35, 70], "O":[90, 40, 49]}
pattern2 = {"W":[245, 190], "E":[25, 30, 35, 70], "L":[150, 130, 190, 120], "N":[25, 30, 35, 70], "S":[90, 40, 49]}
